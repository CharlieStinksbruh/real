...
import { Search, Globe, Zap, Smartphone, FileText, AlertTriangle, CheckCircle, XCircle, Download, ExternalLink, BarChart3, Users, Share2 } from 'lucide-react';
...
function App() {
  ...
  const analyzeWebsite = async () => {
    ...
    // Start crawl progress simulation
    setCrawlProgress({
      isActive: true,
      currentPage: url,
      pagesFound: 1,
    ...
    try {
      ...
      const progressInterval = setInterval(() => {
        setCrawlProgress(prev => ({
          ...prev,
          pagesFound: Math.min(prev.pagesFound + Math.floor(Math.random() * 3), 15),
          pagesCrawled: Math.min(prev.pagesCrawled + 1, prev.pagesFound),
          currentPage: `${url}${['/about', '/contact', '/blog', '/services', '/products'][Math.floor(Math.random() * 5)]}`
        }));
      }, 500);
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              ...
              <p className="mt-2 text-sm text-gray-500">
                We'll crawl your website and analyze up to 50 pages for comprehensive SEO insights
              </p>
            </div>
